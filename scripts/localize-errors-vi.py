#!/usr/bin/env python3
"""Replace English user-facing API/domain error strings with Vietnamese."""
...
# Exact string replacements (order: longer first where needed)
REPLACEMENTS: list[tuple[str, str]] = [
    # Global handler titles / details
    ...
    # Chat / roommate domain
    ("Invitation id and rental post id must not be empty.", "Mã lời mời và mã tin đăng không được trống."),
    ...
    ("Conversation id and sender id must not be empty.", "Mã cuộc trò chuyện và người gửi không được trống."),
    ...
    # AI / chatbot / payments infra (user-facing)
    (
    ...
    # Generic templates used with fieldName
    ...
    (
        ...
        '$"{fieldName} phải từ 1 đến 5."',
    ),
]

# Additional Application-layer validation strings often in dictionaries
EXTRA = [
...
def main() -> None:
    ...
    for path in ROOT.rglob("*.cs"):
        ...
        if text != original:
            ...
            changed_files += 1
            total_hits += hits
            print(f"updated {path.relative_to(ROOT)} ({hits})")

    print(f"\nDone: {changed_files} files, {total_hits} replacements")
